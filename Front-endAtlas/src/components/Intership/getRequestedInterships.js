  ...
  async function getData() {
    if (userdata != undefined) {
      const result = await fetch(
        `http://localhost:8080/api/request_student_intership/${id}`,
        {
          //method: "GET",
          headers: {
  ...
  return (
    <div className="main">
      ...
      {!isEmptyObject(userdata) ? (
        ...
        <div>
          <h1> You does not have requested Intership </h1>
        </div>
      )}
    </div>
  );
...
export default GetRequestedInterships;
